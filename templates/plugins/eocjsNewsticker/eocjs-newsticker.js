    ...
    return this.each(function () {
      ...
      function start() {
        ...
        if (settings.type === "static") {
          ...
          run(content, settings.timeout * 0);
        } else if (settings.type === "ajax") {
          container.prepend('<div class="eocjs-newsticker-loader"></div>');
          $.when(ajax(updateSource(settings.source), settings.fetch)).done(
            function (data) {
              setContent(data);
              container
                .find(".eocjs-newsticker-loader")
              ...
              setInterval(function () {
                $.when(
                  ajax(updateSource(settings.source), settings.fetch)
                ).done(function (data) {
                  setContent(data);
                  oneNeedsUpdate = true;
                  twoNeedsUpdate = true;
        ...
        if ($.isPlainObject(data) && !$.isEmptyObject(data)) {
          for (let property in data) {
            if (data.hasOwnProperty(property)) {
              if (content === "") {
                content =
                  convert("prefix") + data[property] + convert("suffix");
              } else {
                content =
